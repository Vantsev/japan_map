...
<style>
  ...
  *{box-sizing:border-box}
  ...
  .wrap{max-width:900px;margin:0 auto;padding:16px 12px 60px;text-align:center}
  ...
  .topbar{position:fixed;top:10px;left:12px;right:12px;display:flex;justify-content:space-between;align-items:flex-start;z-index:30}
  .tleft{display:flex;flex-direction:column;gap:6px;align-items:flex-start}
  ...
  .side{text-align:left;font-size:12px;min-width:150px}
  .side.hide{display:none}
  #stats h3{font-size:13px;margin:0 0 10px}
  .stat{display:flex;justify-content:space-between;gap:10px;padding:6px 8px;border-bottom:1px solid #e7e0cf}
  .stat b{font-weight:800}
  .backbtn{font:inherit;font-size:13px;font-weight:700;padding:7px 14px;border-radius:10px;border:2px solid var(--line);background:#fff;cursor:pointer;margin:2px 0 6px}
  ...
  .mapbox{position:relative;flex:1 1 360px;max-width:640px;width:100%}
...
<body>
<div class="topbar"><div class="tleft"><a href="/">← карта префектур</a><button id="back" style="display:none">← назад к уровню по городам</button></div><span id="acct"></span></div>
<div class="wrap">
  ...
  <div class="layout">
    <div class="legend side" id="legend"></div>
    <div class="side" id="stats"></div>
    <div class="mapbox">
...
function renderStats(){
  const el=document.getElementById('stats'); el.classList.remove('hide');
  // per-prefecture marked counts
  const byPref={}; for(const code in cities){const pid=String(parseInt(code.slice(0,2),10));byPref[pid]=(byPref[pid]||0)+1;}
  let totDone=0,totAll=0;for(const id in IDX){totAll+=IDX[id].n;}
  for(const id in byPref)totDone+=byPref[id];
  const started=Object.keys(byPref).length;
  let leadId=null,leadN=0;for(const id in byPref)if(byPref[id]>leadN){leadN=byPref[id];leadId=id;}
  const leadName=leadId?((PREF.find(p=>String(p.id)===leadId)||{}).r||''):'—';
  const pct=totAll?Math.round(totDone/totAll*1000)/10:0;
  el.innerHTML=`<h3>📊 Статистика</h3>`+
    `<div class="stat"><span>Городов отмечено</span><b>${totDone}</b></div>`+
    `<div class="stat"><span>Префектур начато</span><b>${started} / 47</b></div>`+
    `<div class="stat"><span>Лидер</span><b>${esc(leadName)}${leadId?' · '+leadN:''}</b></div>`+
    `<div class="stat"><span>Пройдено Японии</span><b>${pct}%</b></div>`;
}

// ================= PREFECTURE DETAIL =================
async function openPref(id){
  view='detail';curPref=id;tip.style.opacity=0;
  const pr=(PREF.find(p=>p.id===id)||{});
  document.getElementById('crumb').innerHTML='🗾 Япония › '+esc(pr.r||'')+' '+esc(pr.ja||'');
...
function renderLegend(on){
  ...
  if(on)document.getElementById('stats').classList.add('hide');
}
...
function up(e){const quick=Date.now()-downT<400;pts.delete(e.pointerId);if(pts.size<2)pinchD=0;
  if(downEl&&!moved&&quick&&pts.size===0){
    if(view==='over'&&downEl._pid!=null){ if(IDX[downEl._pid])openPref(downEl._pid); else toast('данных этой префектуры пока нет'); }
    else if(view==='detail'&&downEl.dataset.code){ setCity(downEl.dataset.code); }
  }
  downEl=null;}
